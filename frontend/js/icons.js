...
export function icon(name, size = 17) {
  return (
    ...
    `stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${PATHS[name] ?? ""}</svg>`
  );
}
...
export function renderStaticIcons(root = document) {
  for (const element of root.querySelectorAll("[data-icon]")) {
    element.innerHTML = icon(element.dataset.icon, Number(element.dataset.iconSize) || 17);
  }
}
